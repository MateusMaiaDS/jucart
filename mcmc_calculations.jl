function suffstats(tree_residuals::Vector{Float64},X_tree::Matrix{Float64},bs::StandardBartState,bm::BartModel)
    ...
    r_sum = transpose(X_tree)*tree_residuals

    BartSufficientStats(number_leaves,omega,r_sum)
end

function mll(ss::BartSufficientStats,bs::StandardBartState,bm::BartModel,index::Int)
    return -0.5*log(2*pi*bm.hypers.σ_μ^2)+0.5*log(bs.σ/ss.omega[index])+0.5*(ss.r_sum[index]^2)/(bs.σ*ss.omega[index])
end
